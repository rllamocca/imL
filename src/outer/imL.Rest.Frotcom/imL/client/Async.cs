...
namespace imL.Rest.Frotcom
{
    public partial class FrotcomClient
    {
        ...
        {
            return await _CLIENT.PostJsonAsync<Authorize200, Authorize>("authorize", Format.User, true, _ct);
        }
        ...
        public async Task<Authorize200> ValidateTokenAsync(string _token, bool _throw = false)
        {
            try
            {
                ...
                return await _CLIENT.PutJsonAsync<Authorize200, string>("authorize", _token);
            }
            catch (Exception)
            {
                if (_throw)
        ...
        public async Task<IEnumerable<Location200>> GetVehicleLocationsAsync(long _vehicle_id)
        {
            ...
            string _uri = "vehicles/{1}/locations?api_key={0}&df={2}%3a{3}&allPositions=true&loadLastPosition=true";
        ...
        {
            string _uri = "accounts?api_key={0}";
            _uri = string.Format(_uri, Authorize.token);
        ...
        {
            string _uri = "drivers/{0}?api_key={1}";
